...
float** opencl_solver(float** T_imp, int n, int m, int time, params param, int rank, int size, int miniN, float* left_buffer, float* right_buffer)
{
	if (rank == 0 && size == 1)
		miniN = n;
	...
	float* T = copy2DArray(T_imp, n, m)[0];

	float* T_new = new float[total_size];
	...
	queue.enqueueWriteBuffer(dev_h, CL_TRUE, 0, sizeof(params), &param.h);

	cl::Buffer dev_k(context, CL_MEM_READ_WRITE, sizeof(float));
	queue.enqueueWriteBuffer(dev_k, CL_TRUE, 0, sizeof(float), &param.k);

	cl::Buffer dev_dt(context, CL_MEM_READ_WRITE, sizeof(float));
	queue.enqueueWriteBuffer(dev_dt, CL_TRUE, 0, sizeof(float), &param.dt);

	cl::Buffer dev_dz(context, CL_MEM_READ_WRITE, sizeof(float));
	queue.enqueueWriteBuffer(dev_dz, CL_TRUE, 0, sizeof(float), &param.dz);

	cl::Buffer dev_Tb(context, CL_MEM_READ_WRITE, sizeof(float));
	queue.enqueueWriteBuffer(dev_Tb, CL_TRUE, 0, sizeof(float), &param.Tb);

	cl::Buffer dev_T0(context, CL_MEM_READ_WRITE, sizeof(float));
	queue.enqueueWriteBuffer(dev_T0, CL_TRUE, 0, sizeof(float), &param.T0);

	cl::Buffer dev_kL(context, CL_MEM_READ_WRITE, sizeof(float));
	queue.enqueueWriteBuffer(dev_kL, CL_TRUE, 0, sizeof(float), &param.kL);

	cl::Buffer dev_kR(context, CL_MEM_READ_WRITE, sizeof(float));
	queue.enqueueWriteBuffer(dev_kR, CL_TRUE, 0, sizeof(float), &param.kR);

	cl::Buffer dev_kU(context, CL_MEM_READ_WRITE, sizeof(float));
	queue.enqueueWriteBuffer(dev_kU, CL_TRUE, 0, sizeof(float), &param.kU);

	cl::Buffer dev_kD(context, CL_MEM_READ_WRITE, sizeof(float));
	queue.enqueueWriteBuffer(dev_kD, CL_TRUE, 0, sizeof(float), &param.kD);
	...
	float* C = new float[total_size];
	getErrorString(queue.enqueueReadBuffer(dev_T, CL_TRUE, 0, sizeof(float) * n * m, C));
	clock_t end = clock();
...
bool isSupportedOpenCL()
{
	std::vector<cl::Platform> all_platforms;
	cl::Platform::get(&all_platforms);
	return all_platforms.size() > 0;
}